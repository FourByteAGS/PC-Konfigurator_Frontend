...
export const fetchHardwareData = async (hardware, apiFunction, token, filterValues, category, subCategory) => {
    try {
        const url = new URL(`${BASE_URL}${hardware}${apiFunction}${token}${filterValues}`);
        const response = await fetch(url.toString());
